...
export class AutomationService {
  ...
  async processMapping(mapping: TagToPathMapping): Promise<AutomationResult> {
    ...
    try {
      ...
      while (true) {
        const people = await this.client.getPeopleWithTag(mapping.sourceTag, page, perPage);
      ...
      console.log(`Found ${allPeople.length} people with tag "${mapping.sourceTag}"`);
      ...
      const targetPath = await this.client.getPathBySlug(mapping.targetPathSlug);
      ...
      console.log(`Step 3: Getting people currently on path "${targetPath.name}"`);
      const allJourneys = [];
      page = 1;

      while (true) {
        const journeys = await this.client.getPathJourneys(targetPath.id, page, perPage);
      ...
      console.log(`Found ${result.peopleAlreadyOnPath.size} people already on path`);

      // Step 4: Filter to find people who need to be added
      result.peopleToAdd = allPeople.filter(person => !result.peopleAlreadyOnPath.has(person.id));
      console.log(`${result.peopleToAdd.length} people need to be added to the path`);

      // Step 5: Add people to path (in simulation or live mode)
      if (result.peopleToAdd.length > 0) {
        console.log(`Step 5: Adding ${result.peopleToAdd.length} people to path`);
        ...
        for (const person of result.peopleToAdd) {
          try {
            ...
            result.additionResults.push({ person, success: false, error: errorMessage });
      ...
      console.error(`❌ Error processing mapping ${mapping.sourceTag} → ${mapping.targetPathSlug}:`, error);
      throw error;
    }
  }

  async processAllMappings(mappings: TagToPathMapping[]): Promise<AutomationResult[]> {
    console.log(`🚀 Starting automation for ${mappings.length} tag-to-path mappings`);
    ...
    for (const mapping of mappings) {
      try {
        ...
        console.error(`Failed to process mapping ${mapping.sourceTag} → ${mapping.targetPathSlug}:`, error);
    ...
    results.forEach((result, index) => {
      ...
      console.log(`${index + 1}. ${result.mapping.sourceTag} → ${result.mapping.targetPathSlug}:`);
