...
function parseTagMappings(): TagToPathMapping[] {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key.startsWith('NB_MAPPING_') && value) {
      ...
      if (parts.length !== 3) {
        throw new Error(`Invalid tag mapping format for ${key}. Expected "tag_name|path_slug|step_number", got "${value}"`);
      }
      
      const [tagName, pathSlug, stepStr] = parts;
      
      if (!tagName || !pathSlug || !stepStr) {
        throw new Error(`Invalid tag mapping format for ${key}. All parts (tag|path|step) must be non-empty.`);
      }
      
      const stepNumber = parseInt(stepStr, 10);
      
      if (isNaN(stepNumber) || stepNumber < 1) {
        throw new Error(`Invalid step number for ${key}. Expected positive integer, got "${stepStr}"`);
      }
